...
EOF
    [ -e /sys/kernel/debug/sched/migration_cost_ns ] && echo 5000000 > /sys/kernel/debug/sched/migration_cost_ns
    [ -e /sys/kernel/debug/sched/min_granularity_ns ] && echo 10000000 > /sys/kernel/debug/sched/min_granularity_ns
    [ -e /sys/kernel/debug/sched/wakeup_granularity_ns ] && echo 15000000 > /sys/kernel/debug/sched/wakeup_granularity_ns
    sysctl -p > /dev/null
}
...
function Tweaked_BBR {
    ...
    wget https://raw.githubusercontent.com/iamnhx/Bash-Scripts/master/Seedbox%20Provisioning/Miscellaneous/BBR/BBR.sh && chmod +x BBR.sh
